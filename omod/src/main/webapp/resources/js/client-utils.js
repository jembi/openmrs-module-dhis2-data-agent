...
const generateReport = function(baseUrl, reportIndex, reportName) {
    $(`#generate-report-${reportIndex}`).hide();
    $(`#loading-icon-${reportIndex}`).show();
    $(`#report-data-${reportIndex}`).hide();
    ...
    $.get(`${baseUrl}/generateReport.form?reportIndex=${reportIndex}&startDate=${date1}&endDate=${date2}`, function (data, status) {
        $('#report-data-' + reportIndex).html(renderReportData(baseUrl, data, reportIndex));
        $(`#report-data-${reportIndex}`).show();
    });
}

const renderReportData = function(baseUrl, data, reportIndex) {
    $(`#loading-icon-${reportIndex}`).hide();
    ...
    // result += `<button type="button" class="btn button" id="button-cancel-${data.reportName.replace(' ','')}" onclick="generateReport('${baseUrl}', ${reportIndex},${data.reportName.replace(' ','')})">Cancel and rebuild report</button>`;
    ...
    return new Promise((resolve => {
        $.get(baseUrl + '/getReportList.form', function (data, status) {
            ...
            data.forEach(reportName => {
                result += `
                    <div class="row"><div class="col-9 section-title">&nbsp;&nbsp;&nbsp;<img src="${imagePath}/report.png"> &nbsp;&nbsp;&nbsp;${reportName}</div> <div class="col-3"><button type="button" id="generate-report-${i}" class="btn button" onclick="generateReport('${baseUrl}', ${i})">Generate this report</button><div id="loading-icon-${i}" class="spinner-border text-primary" style="display:none" role="status"></div></div></div>
